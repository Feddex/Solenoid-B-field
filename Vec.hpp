...
class VecOperation {
 public:
  static double distance(Vec const &a, Vec const &b);

  // arrow point is the first point(vector) passed, the tail is the second one,
  // this is like a difference bewteen vectors
  static Vec dist_vector(Vec const &a, Vec const &b);

  // this return the cross product vector
  static Vec crossProuct(Vec const &a, Vec const &b);
};
